...
{
    std::vector<std::vector<double> >x(LENGTH,std::vector<double>(LINE,0));
    std::vector<double> y;

    std::ifstream ifstr_data("../data/Lable_guiyihua.txt");
    double d = 0;
    ...
    {        
        ...
        for(int j = 0; j < LINE + 1; ++j)
        {
            ...
            //NUM_data.push_back(d);
            x[i][j] = d;
        }
        x.push_back(NUM_data);
        std::vector<double>().swap(NUM_data);//清空NUM_data的内存
    }
    ifstr_data.close();
}
