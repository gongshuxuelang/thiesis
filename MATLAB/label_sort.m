...
classNoB=importdata('Lable_清洗.txt');
classNo = unique(classNoB,'rows');  %去重函数
fid = fopen('Lable.txt','wb');  %%创建txt文件
fid1 = fopen('Lable_qucong.txt','wb');  %%创建txt文件
fprintf(fid,[repmat('%d\t', 1, size(B,2)), '\n'], B');%%保存数据
fprintf(fid1,[repmat('%d\t', 1, size(B,2)), '\n'], classNo');%%保存数据
fclose(fid);                     %%关闭文件  
fclose(fid1);  